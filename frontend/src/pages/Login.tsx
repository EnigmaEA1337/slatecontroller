...
export default function Login() {
  ...
  const errorMessage =
    login.error instanceof AxiosError && login.error.response?.status === 401
      ? "[ ACCESS DENIED ] Identifiants invalides"
      : login.error
        ? "[ NETWORK ERROR ] Backend injoignable"
        : null;
    ...
    <main className="flex min-h-screen items-center justify-center px-4">
      <div className="cyber-card cyber-card-accent w-full max-w-md p-10">
        ...
        <form onSubmit={onSubmit} className="space-y-5">
          <div>
            <span className="cyber-label mb-2 block">Utilisateur</span>
            <div className="relative">
              <User className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-[color:var(--color-cyber-dim)]" />
              <input
                type="text"
          ...
          <div>
            <span className="cyber-label mb-2 block">Mot de passe</span>
            <div className="relative">
          ...
          >
            {login.isPending ? "// connecting…" : "Se connecter ▸"}
          </button>
        </form>

        <div className="mt-10 flex items-center justify-between">
          <div className="cyber-hatch h-2 w-16" />
          <p className="text-[10px] uppercase tracking-[0.25em] text-[color:var(--color-cyber-dim)]">
            v0.1.0 · GL-BE10000
          </p>
        </div>
      </div>
